        ...
        double thresh)
{
    ...
    for (i = 0; i < res.rows; i++) {
        for (j = 0; j < res.cols; j++) {
            ...
            if (idx != 0)
                idx = 1;
            segmented_points[idx].insert(segmented_points[idx].end(), extendedMap[i][j].begin(), extendedMap[i][j].end());
        }
    }
        ...
        int maxlevel, int radius)
{
    ...
    for (i = 0; i < res.rows; i++) {
        for (j = 0; j < res.cols; j++) {
            idx = res.at<uchar>(i,j);
            histogram[idx].insert(histogram[idx].end(), extendedMap[i][j].begin(), extendedMap[i][j].end());
        }
    }
    ...
    for (i = 0; i < markers.rows; i++) {
        for (j = 0; j < markers.cols; j++) {
            ...
            if (idx > 0 && idx <= compCount) {
                segmented_points[idx-1].insert(segmented_points[idx-1].end(), extendedMap[i][j].begin(), extendedMap[i][j].end());
            }
        }
    }
...
void write3dfiles(vector<vector<cv::Vec3f>> &segmented_points, string &segdir)
{
    ...
    for (i = 0; i < segmented_points.size(); i++) {
        for (vector<cv::Vec3f>::iterator it=segmented_points[i].begin() ; it < segmented_points[i].end(); it++) {
...
int main(int argc, char **argv)
{
    ...
    for(ScanVector::iterator it = Scan::allScans.begin(); it != Scan::allScans.end(); ++it) {
        ...
        if (stype == THRESHOLD) {
            ...
            res = calculatePyrMeanShift(segmented_points, img, fPanorama.getExtendedMap(), maxlevel, radius);
