...
vector<PointXY> path_vectorX, path_vectorZ, lookat_vectorX,
    lookat_vectorZ, ups_vectorX, ups_vectorZ;
vector<Point> cams;
...
void usage(char *prog)
{
  ...
  cout << bold << "OPTIONS" << normal << endl
      << bold << "  -e" << normal << " NR, " << bold << "--end=" << normal
      << "NR" << endl << "         end after scan NR" << endl << endl <<
      bold << "  -f" << normal << " F, " << bold << "--format=" << normal
      << "F" << endl << "         using shared library F for input" << endl
      <<
      "         (chose F from {uos, uos_map, uos_rgb, uos_frames, uos_map_frames, old, rts, rts_map, ifp, riegl_txt, riegl_rgb, riegl_bin, zahn, ply, wrl, xyz, zuf, iais, front, x3d, rxp, ais })"
      << endl << endl << bold << "  -F" << normal << " NR, " << bold <<
      "--fps=" << normal << "NR [default: 20]" << endl <<
      "         will attempt to display points with a framerate of NR" <<
      endl << endl << bold << "  -l" << normal << " FILE, " << bold <<
      "--loadObj=" << normal << "FILE" << endl <<
      "         load objects specified in <FILE>" << endl << endl << endl
      << bold << "  -m" << normal << " NR, " << bold << "--max=" << normal
      << "NR" << endl <<
      "         neglegt all data points with a distance larger than NR 'units'"
      << endl << endl << bold << "  -M" << normal << " NR, " << bold <<
      "--min=" << normal << "NR" << endl <<
      "         neglegt all data points with a distance smaller than NR 'units'"
      << endl << endl << bold << "  -O" << normal << "NR (optional), " <<
      bold << "--octree=" << normal << "NR (optional)" << endl <<
      "         use randomized octree based point reduction (pts per voxel=<NR>)"
      << endl << "         requires " << bold << "-r" << normal << " or "
      << bold << "--reduce" << endl << endl << bold << "  -o" << normal <<
      " NR, " << bold << "--origin=" << normal << "NR (optional)" << endl
      << "         sets the starting and reset position to: " << endl <<
      "           0 = the origin of the coordinate system (default)" <<
      endl <<
      "           1 = the position of the first scan (default if --origin is in argument list)"
      << endl << "           2 = the center of the first scan" << endl <<
      endl << bold << "  -S, --scanserver" << normal << endl <<
      "           Use the scanserver as an input method and handling of scan data"
      << endl << endl << bold << "  -r" << normal << " NR, " << bold <<
      "--reduce=" << normal << "NR" << endl <<
      "         turns on octree based point reduction (voxel size=<NR>)" <<
      endl << endl << bold << "  -s" << normal << " NR, " << bold <<
      "--start=" << normal << "NR" << endl <<
      "         start at scan NR (i.e., neglects the first NR scans)" <<
      endl << "         [ATTENTION: counting naturally starts with 0]" <<
      endl << endl << bold << "  -C" << normal << " NR, " << bold <<
      "--scale=" << normal << "NR" << endl <<
      "         scale factor to use (default: 0.01), modifies movement speed etc. "
      << endl <<
      "         use 1 when point coordinates are in m, 0.01 when in cm and so forth. "
      << endl << "         " << endl << endl << bold <<
      "  -R, --reflectance, --reflectivity" << normal << endl <<
      "         use reflectivity values for coloring point clouds" << endl
      << "         only works when using octree display" << endl << endl <<
      ...
      "         only works when using octree display" << endl << bold <<
      "  -c, --color" << endl << normal <<
      "         use color RGB values for coloring point clouds" << endl <<
      bold << "  --saveOct" << endl << normal <<
      "         stores all used scans as octrees in the given directory" <<
      endl <<
      "         All reflectivity/amplitude/deviation/type settings are stored as well."
      << endl << "         only works when using octree display" << endl <<
 ...
 */
int parseArgs(int argc, char **argv, string & dir, int &start, int &end,
              int &maxDist, int &minDist, double &red, bool & readInitial,
              int &octree, PointType & ptype, float &fps, string & loadObj,
              ...
              IOType & type, bool & scanserver)
{
  ...
  while ((c =
          getopt_long(argc, argv, "F:f:s:e:r:m:M:O:o:l:C:SwtRadhTc",
                      longopts, NULL)) != -1) {
    switch (c) {
    case 's':
      w_start = atoi(optarg);
 ...
 */
int readFrames(string dir, int start, int end, bool readInitial,
               IOType & type)
{
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (identity) {
        ...
        EulerToMatrix4(Scan::allScans[index]->get_rPos(),
                       Scan::allScans[index]->get_rPosTheta(), transMat);
      }
...
void initShow(int argc, char **argv)
{
  ...
  cout << "(wx)show - A highly efficient 3D point cloud viewer" << endl
      ...
      << "    University of Osnabrueck, Germany, 2006 - 2009" << endl <<
      endl;
  ...
  parseArgs(argc, argv, dir, start, end, maxDist, minDist, red,
            readInitial, octree, pointtype, idealfps, loadObj, loadOct,
            saveOct, origin, scale, type, scanserver);
...
//  oldfardistance *= scale;
  ...
  for (ScanVector::iterator it = Scan::allScans.begin();
    ...
    if (red > 0) {
      ...
      if (scanserver) {
        dynamic_cast <ManagedScan*>
            (scan)->setShowReductionParameter(red, octree);
      } else {
  ...
  if (loadOct)
    cout <<
        "Loading octtrees from file where possible instead of creating them from scans."
        << endl;
  ...
  for (unsigned int i = 0; i < Scan::allScans.size(); ++i) {
    ...
    try {
      if (red > 0) {                // with reduction, only xyz points
        ...
        tree =
            new compactTree(PointerArray <double> (xyz_r).get(),
                            xyz_r.size(), voxelSize, pointtype, cm);
      ...
      cout << "Scan " << i <<
          " could not be loaded into memory, stopping here." << endl;
      break;
    }
    ...
    Show_BOctTree <sfloat> *tree =
        new Show_BOctTree <sfloat> (scan, data_oct, cm);
  ...
  if (scanserver) {
    ...
    for (vector <colordisplay*>::iterator it = octpts.begin();
      ...
      if (!stop) {
        ...
        try {
          Show_BOctTree <sfloat> *stree =
              dynamic_cast <Show_BOctTree<sfloat>*> (*it);
    ...
    if (stop)
      octpts.erase(it_remove_first, octpts.end());
