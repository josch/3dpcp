...
void DrawPoints(GLenum mode, bool interruptable)
{
  ...
  if (LevelOfDetail > max)
    LevelOfDetail = max;
  else if (LevelOfDetail < min)
    LevelOfDetail = min;
  ...
  if (frameNr != 0) {
...
#else
    for (int iterator = (int)Scan::allScans.size() - 1; iterator >= 0;
         iterator--) {
#endif
      // ignore scans that don't have any frames associated with them
      if ((unsigned int) iterator >= MetaMatrix.size())
        continue;
      // set usable frame
      double *frame;
      Scan::AlgoType type;
      ...
      for (unsigned int jterator = 0;
           jterator < vvertexArrayList[iterator].size(); jterator++) {

        if ((jterator == 0)
            && vvertexArrayList[iterator][jterator]->numPointsToRender >
            0) {
          cm->selectColors(type);
        }
    ...
    if (mode == GL_SELECT) {
      ...
      // ------------------
      GLuint name = 0;
      for (int iterator = (int)octpts.size() - 1; iterator >= 0;
           iterator--) {
        glPushMatrix();
        ...
        for (set<sfloat*>::iterator it =
             selected_points[iterator].begin();
...
void DrawPath()
{
  ...
  for (unsigned int j = 0; j < path_vectorX.size(); j++) {
    ...
    // set the points
    glVertex3f(path_vectorX.at(j).x, path_vectorX.at(j).y,
               path_vectorZ.at(j).y);
  }
  glEnd();
  ...
  for (unsigned int j = 0; j < lookat_vectorX.size(); j++) {
    ...
    //set the points
    glVertex3f(lookat_vectorX.at(j).x, lookat_vectorX.at(j).y,
...
void DisplayItFunc(GLenum mode, bool interruptable)
{
  ...
  if (haveToUpdate == 6 && path_iterator < path_vectorX.size()) {
    gluLookAt(path_vectorX.at(path_iterator).x,
              ...
              ups_vectorX.at(path_iterator).x -
              path_vectorX.at(path_iterator).x,
              ups_vectorX.at(path_iterator).y -
              path_vectorX.at(path_iterator).y,
              ups_vectorZ.at(path_iterator).y -
              path_vectorZ.at(path_iterator).y);
  } else {
  ...
  if (show_fog > 0) {
    ...
    glEnable(GL_FOG);
    {
      ...
      if (show_fog == 1) {
        fogMode = GL_EXP;
        fardistance =
            min(5.54517744 / fogDensity, (double) maxfardistance);
      } else if (show_fog == 2) {
        fogMode = GL_EXP2;
        fardistance =
            min(sqrt(5.54517744) / fogDensity, (double) maxfardistance);
      } else if (show_fog == 3) {
        fogMode = GL_LINEAR;
        fardistance = 32000.0;
      } else if (show_fog == 4) {
...
void topView()
{
  static GLdouble save_qx, save_qy, save_qz, save_qangle, save_X, save_Y,
      save_Z;
             ...
             sf, double fD, bool inv)
{
  ...
  showTopView = sTV, cameraNavMouseMode = cNMM;
  pzoom = pzoom_new;
...
/*  if (haveToUpdate == 7) {
  ...
  if (haveToUpdate == 3) {
    ...
    if (save_animation) {
      string filename =
          scan_dir + "animframe" + to_string(frameNr, 5) + ".ppm";
      ...
      string jpgname =
          scan_dir + "animframe" + to_string(frameNr, 5) + ".jpg";
      string systemcall =
          "convert -quality 100 -type TrueColor " + filename + " " +
          jpgname;
  ...
  if (haveToUpdate == 6) {
    ...
    if (path_iterator < path_vectorX.size()) {        // standard animation case
      ...
      if (save_animation) {
        string filename =
            scan_dir + "animframe" + to_string(path_iterator, 5) + ".ppm";
        string jpgname =
            scan_dir + "animframe" + to_string(path_iterator, 5) + ".jpg";
        cout << "written " << filename << " of " << path_vectorX.
            size() << " files" << endl;
        glWriteImagePPM(filename.c_str(), factor, 0);
        string systemcall =
            "convert -quality 100 " + filename + " " + jpgname;
        system(systemcall.c_str());
        systemcall = "rm " + filename;
        system(systemcall.c_str());
        haveToUpdate = 6;
...
/*    for(int iterator = (int)octpts.size()-1; iterator >= 0; iterator--) {
    ...
    for (int iterator = (int)octpts.size() - 1; iterator >= 0; iterator--) {
      ...
      if (select_voxels) {
        ...
        if (sp != 0) {
          cout << "Selected point: " << sp[0] << " " << sp[1] << " " <<
              sp[2] << endl;

          if (sp2 != 0) {
            cout << "Distance to last point: "
                << sqrt(sqr(sp2[0] - sp[0]) + sqr(sp2[1] - sp[1]) +
                        sqr(sp2[2] - sp[2])) << endl;
          }
          sp2 = sp;
...
//    gluPickMatrix((GLdouble)x, (GLdouble)(viewport[3]-y), 10.0, 10.0, viewport);
    gluPickMatrix((GLdouble) x, (GLdouble) (viewport[3] - y),
                  brush_size * 2, brush_size * 2, viewport);
...
void moveCamera(double x, double y, double z, double rotx, double roty,
                double rotz)
{
  interruptDrawing();
  double mat[9];
...
void glDumpWindowPPM(const char *filename, GLenum mode)
{
  ...
  fp << "P6" << endl <<
      ...
      << endl << win_width << " " << win_height << " " << UCHAR_MAX <<
      endl;
    ...
    for (j = 0; j < win_width; j++) {        // For each column
      for (k = 0; k < RGB; k++) {        // For each RGB component
        ...
        ibuffer[l++] = (unsigned char)
            *(buffer +
              (RGBA * ((win_height - 1 - i) * win_width + j) + k));
      }                                // end RGB
    }                                // end column
  }                                // end row
  ...
  fp.write((const char *) ibuffer,
           sizeof(unsigned char) * (RGB * win_width * win_height));
  fp.close();
  fp.clear();
  delete[]buffer;
  delete[]ibuffer;
...
void glWriteImagePPM(const char *filename, int scale, GLenum mode)
{
  ...
  for (int i = 0; i < scale; i++) {
    ...
    for (int j = 0; j < scale; j++) {
      ...
      label = false;
      if (!showTopView) {
        glFrustum(neardistance * width,
                  neardistance * (width + part_width),
                  neardistance * (height),
                  neardistance * (height + part_height), neardistance,
                  fardistance);
        glMatrixMode(GL_MODELVIEW);
        if (i == 0 && j == 0) {
          label = true;
        ...
        glOrtho(width, width + part_width,
                height, height + part_height, 1.0, 32000.0);
        glMatrixMode(GL_MODELVIEW);
        if (i == 0 && j == 0) {
          label = true;
        }
      ...
      glFinish();
      glReadBuffer(buffermode);
      glReadPixels(0, 0, win_width, win_height, GL_RGBA, GL_UNSIGNED_BYTE,
                   buffer);
      ...
      for (m = 0; m < win_height; m++) {        // For each row
        for (o = 0; o < win_width; o++) {        // For each column
          for (k = 0; k < RGB; k++) {        // For each RGB component
            int l =
                (k +
                 RGB * (image_width * ((scale - 1 - i) * win_height + m) +
                        j * win_width + o));
            ibuffer[l] =
                (unsigned char) *(buffer +
                                  (RGBA *
                                   ((win_height - 1 - m) * win_width + o) +
                                   k));
          }                        // end RGB
        }                        // end column
      }
  ...
  // Write a proper P6 PPM header
  fp << "P6" << endl <<
      "# CREATOR: 3D_Viewer by Dorit Borrmann, Jacobs University Bremen gGmbH"
      << endl << image_width << " " << image_height << " " << UCHAR_MAX <<
      endl;
...
Done:
  ...
  cout << "Erasing " << endl;
  for (set<sfloat*>::iterator it = unsel_points.begin(); it != unsel_points.end(); it++) {        // iterate to the index as indicated by the name *ptr
...
/**
  ...
  * neighboring cameras.
  */
void calcInterpolatedCameras(vector<PointXY> vec1,
                             vector<PointXY> vec2)
{
  ...
  for (unsigned int i = 0; i < vec1.size() - 1; i++) {
    ...
    for (int j = 0; j < im_per_cam; j++) {
      ...
      NurbsPath::camRatio.
          push_back((distance2 +
                     ((double) j * (curr_dist / (double) im_per_cam))) /
                    (distance + 0.1));
    }
...
void calcPointSequence(vector<int> &sequence, int frameNr)
{
  ...
  for (unsigned int i = 0; i < octpts.size(); i++) {
    ...
    y = frame[13];
    z = frame[14];
    dists.push_back(pair < double,
                    int >(sqr(X + x) + sqr(Y + y) + sqr(Z + z), i));
  }
