...
void calcUpPath()
{
  ...
  ups_vectorX =
      cam_nurbs_path.getNurbsPath(ups_listXY, nr_interpolations,
                                  inter_by_dist);
  ups_vectorZ =
      cam_nurbs_path.getNurbsPath(ups_listXZ, nr_interpolations,
                                  inter_by_dist);
}
...
void calcLookAtPath()
{
  ...
  // now get the nurbs path for the individual xy and xz plane
  lookat_vectorX =
      cam_nurbs_path.getNurbsPath(lookat_listXY, nr_interpolations,
                                  inter_by_dist);
  lookat_vectorZ =
      cam_nurbs_path.getNurbsPath(lookat_listXZ, nr_interpolations,
                                  inter_by_dist);
}
...
void calcPath()
{
  ...
  // now get the nurbs path for the individual xy and xz plane
  path_vectorX =
      cam_nurbs_path.getNurbsPath(path_listXY, nr_interpolations,
                                  inter_by_dist);
  path_vectorZ =
      cam_nurbs_path.getNurbsPath(path_listXZ, nr_interpolations,
                                  inter_by_dist);
}
...
void loadPose(int dummy)
{
  ...
  setView(euler, quat, mouseRotX, mouseRotY, mouseRotZ, cangle,
          showTopView, cameraNavMouseMode, pzoom, show_points, show_path,
          show_cameras, pointsize, show_fog, fogDensity, invert);
...
void savePose(int dummy)
{
  ...
  posefile << showTopView << " " << cameraNavMouseMode << " " << pzoom <<
      endl;
  posefile << show_points << " " << show_path << " " << show_cameras << " "
      << pointsize << endl;
